...
def build_param_lr_groups(model, cfg):
    ...
    weight_decay_cfg = cfg.trainer.get("weight_decay_groups", {})
    if not hasattr(weight_decay_cfg, "get"):
        weight_decay_cfg = {}
    base_weight_decay = weight_decay_cfg.get("base", cfg.trainer.optimizer.weight_decay)
    ...
    for module_name, lr in lr_cfg.items():
        ...
        try:
            ...
            if params:  # only add param group if there are trainable parameters
                weight_decay = weight_decay_cfg.get(module_name, base_weight_decay)
                param_groups.append(
                    {
                        "params": params,
                        "lr": lr,
